...
export default function PanelSelectedRegion({
  allProgrammes,
  allProjects,
  allRegionsInfo,
  allCountriesInfo,
  ...
  React.useEffect(() => {
    ...
    progIDs?.forEach((progId) => {
      ...
      let projects = programmeInfo.projects.map((projID) =>
        allProjects.find((pJ) => pJ.ID === projID)
      );
    ...
    <>
      <div className="TM_row">
        <div className="TM_col-8">
          <h2>{regionInfo?.title}</h2>
          <p className="">{getCountry()}</p>
          <small>{programmes?.length} programmes</small>
        </div>
      </div>
      <div className="TM_row projects-by-programme"></div>
