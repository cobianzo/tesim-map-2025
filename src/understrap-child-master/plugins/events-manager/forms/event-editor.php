...
<form enctype='multipart/form-data' id="event-form" class="em-event-admin-editor <?php if( $EM_Event->is_recurring() ) echo 'em-event-admin-recurring' ?>" method="post" action="<?php echo esc_url(add_query_arg(array('success'=>null))); ?>">
	<div class="wrap">
		...
		<div class="inside event-form-name">
			...
			<br />
			<?php esc_html_e('The event name. Example: Joint Monitoring Committee Meeting', 'events-manager'); ?>
		...
		<div class="inside event-form-details">
			...
			<div class="event-extra-details">
				...
				<?php /* <?php if(get_option('dbem_categories_enabled')) { em_locate_template('forms/event/categories-public.php',true); }  ?>*/ ?>
			</div>
	...
	<p class="submit">
	    ...
	    <input type='submit' class='btn btn-outline-primary marginRight' value='<?php echo esc_attr(sprintf( __('Submit %s','events-manager'), __('Event','events-manager') )); ?>' />
	    <?php else: ?>
	    <input type='submit' class='btn btn-outline-primary marginRight' value='<?php echo esc_attr(sprintf( __('Update %s','events-manager'), __('Event','events-manager') )); ?>' />
	    <?php endif; ?>
	</p>
	<br>
	<br>
